...
class HiDreamModel(BaseModel):
    ...
    def _patch_hidream_attention(self):
        """Adaugă un patch pentru problema spatial_norm din HiDreamAttention"""
        try:
            # Încercăm să importăm clasa HiDreamAttention
            from diffusers.pipelines.hidream_image.attention_processor import HiDreamAttention
            
            # Verificăm direct clasa din attention_processor.py
            from diffusers.models.attention_processor import AttnProcessor2_0
            
            # Adăugăm metoda spatial_norm ca atribut
            HiDreamAttention.spatial_norm = None
            
            # Patch și pentru __call__ pentru a gestiona parametrul use_spatial_norm
            original_call = HiDreamAttention.__call__
            
            def patched_call(self, *args, **kwargs):
                # Eliminăm orice referință la use_spatial_norm
                if 'use_spatial_norm' in kwargs:
                    kwargs.pop('use_spatial_norm')
                return original_call(self, *args, **kwargs)
            
            # Înlocuim metoda __call__ cu versiunea noastră patch-uită
            HiDreamAttention.__call__ = patched_call
            
            # Verificăm și câmpurile din AttnProcessor2_0
            if hasattr(AttnProcessor2_0, 'spatial_norm'):
                logger.info("AttnProcessor2_0 already has spatial_norm attribute.")
            else:
                AttnProcessor2_0.spatial_norm = None
                logger.info("Added spatial_norm attribute to AttnProcessor2_0.")
                
            logger.info("Successfully patched HiDreamAttention and related classes")
            return True
                    
        except (ImportError, AttributeError) as e:
            logger.warning(f"Could not apply HiDreamAttention patch: {e}")
            return False
    ...
    def load(self) -> bool:
        ...
        # Patch pentru problema spatial_norm înainte de a importa pipeline-ul
        self._patch_hidream_attention()
        ...
        try:
            ...
            # Timp total de încărcare
            load_time = time.time() - start_time
            logger.info(f"HiDream model '{self.model_id}' and components loaded successfully in {load_time:.2f} seconds.")
            return True
    ...
    def process(
        ...
        **kwargs, # Allows passing other pipeline-specific parameters
        ...
        pipeline_args = {
            ...
            "generator": gen,
            # Dezactivăm use_spatial_norm pentru a evita eroarea
        ...
        # Asigurăm că use_spatial_norm rămâne False chiar dacă e furnizat în kwargs
