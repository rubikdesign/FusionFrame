...
class RemovalPipeline(BasePipeline):
    ...
    def process(self,
               ...
               remove_artifacts: bool = True,  # Default true pentru removal
               **kwargs) -> Dict[str, Any]:
        ...
        if not inpainting_prompt_base or len(inpainting_prompt_base) < 10: 
            inpainting_prompt_base = "empty area, clean background"

        # Folosim direct PromptEnhancer
        enhanced_prompt = self.prompt_enhancer.enhance_prompt(
            prompt=inpainting_prompt_base, 
        ...
        # 5. Apel model principal (Inpainting) cu tratare OOM
        ...
        try:
            ...
            if output_dict.get('success'):
                ...
                return {
                 ...
                 msg = f"Removal failed: {output_dict.get('message')}"
                 logger.error(msg)
                 return {'result_image': pil_image, 'mask_image': mask_pil, 'operation': operation, 'message': msg, 'success': False}
        except Exception as e:
            msg = f"Runtime error during removal pipeline: {e}"
